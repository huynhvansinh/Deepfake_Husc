...
OUTPUT_DIR = "./training_outputs/efficientnet_b4_ffpp_optimized"

BEST_MODEL_PATH = os.path.join(OUTPUT_DIR, "best_model_1.pth")
LAST_MODEL_PATH = os.path.join(OUTPUT_DIR, "last_model_1.pth")
HISTORY_PATH = os.path.join(OUTPUT_DIR, "training_history_1.csv")

CONFUSION_MATRIX_CSV_PATH = os.path.join(OUTPUT_DIR, "confusion_matrix_test_1.csv")
CONFUSION_MATRIX_NPY_PATH = os.path.join(OUTPUT_DIR, "confusion_matrix_test_1.npy")

IMAGE_SIZE = 224

# B4 với log của anh còn dư VRAM, có thể thử 16.
# Nếu CUDA out of memory thì giảm về 8.
BATCH_SIZE = 16

NUM_EPOCHS = 30
LEARNING_RATE = 1e-4

# Tăng nhẹ weight decay để giảm overfit
WEIGHT_DECAY = 2e-4
...
# Dropout classifier để giảm overfit nhẹ
DROPOUT_RATE = 0.4

# Early stopping để không train quá lâu khi val_f1 không cải thiện
EARLY_STOPPING_PATIENCE = 5

# Scheduler: nếu val_f1 không cải thiện, giảm LR
...
# TRANSFORMS
# =========================================================
# Augmentation tăng nhẹ để giảm dao động/overfit,
# nhưng không quá mạnh để tránh phá artifact deepfake.
...
def build_model(num_classes=2, freeze_backbone=False):
    ...
    # Thêm Dropout để giảm overfit nhẹ
...
def main():
    ...
    print("Model: EfficientNet-B4 Optimized")
    ...
    print(f"SCHEDULER: ReduceLROnPlateau(mode='max', factor={SCHEDULER_FACTOR}, patience={SCHEDULER_PATIENCE})")
    print("=================================\n")
    ...
    # AdamW tốt hơn Adam khi fine-tune pretrained model + weight decay
    ...
    # Giảm LR khi Val F1 không cải thiện
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # Scheduler cập nhật theo Val F1
        scheduler.step(val_metrics["f1"])
        new_lr = get_current_lr(optimizer)

        if new_lr < current_lr:
            print(f"Learning rate giảm: {current_lr:.8f} -> {new_lr:.8f}")
        ...
        history.append({
            ...
            "best_val_f1_so_far": max(best_val_f1, val_metrics["f1"])
        })
        ...
        if val_metrics["f1"] > best_val_f1:
            ...
            print(f"Best Val F1 mới: {best_val_f1:.4f} tại epoch {best_epoch}")

        else:
            no_improve_count += 1
            print(f"Không cải thiện Val F1: {no_improve_count}/{EARLY_STOPPING_PATIENCE}")
            print(f"Best Val F1 hiện tại: {best_val_f1:.4f} tại epoch {best_epoch}")
        ...
        if no_improve_count >= EARLY_STOPPING_PATIENCE:
            print("Early stopping: dừng train do Val F1 không cải thiện.")
            break
